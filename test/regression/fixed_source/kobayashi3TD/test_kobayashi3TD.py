...
def test_serial(get_file, mode):
    file = get_file("problems/fixed_source/kobayashi3TD_input.py")
    os.system("python {} --particles=40 --mode={}".format(file, mode))
...
def test_processor(get_file, nprocess, mode):
    ...
    os.system(
        "python {} --particles=40 --mode={} --file=output_serial".format(file, mode)
    )
