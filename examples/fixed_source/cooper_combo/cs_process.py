...
with h5py.File("output.h5", "r") as f:
    ...
    plt.imshow(np.rot90(recon), extent=[0, 4, 0, 4])
    plt.xlabel("x [cm]")
    plt.ylabel("y [cm]")
    plt.title(
    ...
    plt.imshow(np.rot90(mesh_results), extent=[0, 4, 0, 4])
    plt.xlabel("x [cm]")
    plt.ylabel("y [cm]")
    plt.title(f"MC/DC Full Sim Results")
    plt.colorbar()
    # plt.savefig('MCDC_full_sim_cooper_combo.png')
    plt.show()
...
for i in range(4):
    for j in range(4):
        ax = axes[i, j]
        im = ax.imshow(np.rot90(reconstruct(l_array[i * 4 + j])), extent=[0, 4, 0, 4])
...
plt.tight_layout()
# plt.savefig('BPDN_lambda_testing_cooper_combo.png')
plt.show()
...
plt.tight_layout()
# plt.savefig('Relative_error_vs_lambda_cooper_combo.png')
plt.show()
