...
time_filter   = mcdc.FilterTime([0.0]+list(np.logspace(-9,-0.5,50)))

T = mcdc.Tally('tally', scores=['flux-edge'], energy_filter=energy_filter,
...
# Set simulator
simulator = mcdc.Simulator(cells, sources, tallies=tallies, N_hist=1E6, speed=v,
                           decay=lamd)
simulator.set_pct(census_time=np.array([np.logspace(-9,-0.5,50)[-1]]))
