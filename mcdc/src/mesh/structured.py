...
def get_indices(particle_container, mesh):
    ...
    ix = _grid_index(x, ux, mesh["x"], Nx + 1)
    iy = _grid_index(y, uy, mesh["y"], Ny + 1)
    iz = _grid_index(z, uz, mesh["z"], Nz + 1)
    it = _grid_index(
        t, 1.0, mesh["t"], Nt, COINCIDENCE_TOLERANCE_TIME
...
def get_crossing_distance(particle_arr, speed, mesh):
    ...
    d = INF
    d = min(d, _grid_distance(x, ux, mesh["x"], Nx + 1))
    d = min(d, _grid_distance(y, uy, mesh["y"], Ny + 1))
    d = min(d, _grid_distance(z, uz, mesh["z"], Nz + 1))
    d = min(
...
@njit
def _grid_index(value, direction, grid, length, tolerance=COINCIDENCE_TOLERANCE):
    """
...
@njit
def _grid_distance(value, direction, grid, length, tolerance=COINCIDENCE_TOLERANCE):
    """
