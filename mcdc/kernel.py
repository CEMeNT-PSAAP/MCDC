...
def manage_IC_bank(mcdc):
    # Buffer bank
    buff_n = np.zeros(mcdc['technique']['IC_bank_neutron_local']
                          ['content'].shape[0], dtype=type_.neutron)
    buff_p = np.zeros(mcdc['technique']['IC_bank_precursor_local']
                          ['content'].shape[0], dtype=type_.precursor)
    ...
    if mcdc['technique']['IC_resample']:
        ...
        # Set actual bank
        for i in range(Nn):
            mcdc['technique']['IC_bank_neutron_local']['content'][i] =\
                buff_n[i]
        ...
        # Set actual bank
        for i in range(Np):
            mcdc['technique']['IC_bank_precursor_local']['content'][i] =\
                buff_p[i]
    ...
    if mcdc['mpi_master']:
        ...
        for i in range(Nn):
            mcdc['technique']['IC_bank_neutron']['content'][start_n + i] =\
                buff_n[i]
        for i in range(Np):
            mcdc['technique']['IC_bank_precursor']['content'][start_p + i] =\
                buff_p[i]
...
def bank_IC(P, mcdc):
    ...
    if rng(mcdc) < Pp:
        ...
        precursor =\
            mcdc['technique']['IC_bank_precursor_local']['content'][idx]
        precursor['x'] = P['x']
        precursor['y'] = P['y']
        precursor['z'] = P['z']
...
def surface_evaluate(P, surface, trans):
    ...
    return result + A * x * x + B * y * y + C * \
        z * z + D * x * y + E * x * z + F * y * z
...
def score_crossing_x(P, t, x, y, z, mcdc):
    ...
    flux = P['w'] / abs(P['ux'])
    if tally['flux_x']:
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['flux_x'])
    if tally['density_x']:
        flux /= material['speed'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['density_x'])
    if tally['fission_x']:
        flux *= material['fission'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['fission_x'])
    if tally['total_x']:
        flux *= material['total'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['total_x'])
    if tally['current_x']:
        score_current(s, g, t, x, y, z, flux, P,
                      tally['score']['current_x'])
    if tally['eddington_x']:
        score_eddington(s, g, t, x, y, z, flux, P,
                        tally['score']['eddington_x'])
...
def score_crossing_y(P, t, x, y, z, mcdc):
    ...
    if tally['density_y']:
        flux /= material['speed'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['density_y'])
    if tally['fission_y']:
        flux *= material['fission'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['fission_y'])
    if tally['total_y']:
        flux *= material['total'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['total_y'])
    if tally['current_y']:
        score_current(s, g, t, x, y, z, flux, P,
                      tally['score']['current_y'])
    if tally['eddington_y']:
        score_eddington(s, g, t, x, y, z, flux, P,
                        tally['score']['eddington_y'])
...
def score_crossing_z(P, t, x, y, z, mcdc):
    ...
    if tally['density_z']:
        flux /= material['speed'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['density_z'])
    if tally['fission_z']:
        flux *= material['fission'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['fission_z'])
    if tally['total_z']:
        flux *= material['total'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['total_z'])
    if tally['current_z']:
        score_current(s, g, t, x, y, z, flux, P,
                      tally['score']['current_z'])
    if tally['eddington_z']:
        score_eddington(s, g, t, x, y, z, flux, P,
                        tally['score']['eddington_z'])
...
def score_crossing_t(P, t, x, y, z, mcdc):
    ...
    if tally['density_t']:
        flux /= material['speed'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['density_t'])
    if tally['fission_t']:
        flux *= material['fission'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['fission_t'])
    if tally['total_t']:
        flux *= material['total'][g]
        score_flux(s, g, t, x, y, z, mu, azi, flux,
                   tally['score']['total_t'])
    if tally['current_t']:
        score_current(s, g, t, x, y, z, flux, P,
                      tally['score']['current_t'])
    if tally['eddington_t']:
        score_eddington(s, g, t, x, y, z, flux, P,
                        tally['score']['eddington_t'])
...
def global_tally(P, distance, mcdc):
    ...
    if mcdc['technique']['IC_generator']:
        ...
        for j in range(J):
            total += nu_d[j] / decay[j]
        mcdc['technique']['IC_tally_C'] += flux * \
            total * SigmaF / mcdc['k_eff']
...
def global_tally_closeout_history(mcdc):
    ...
    with objmode():
        MPI.COMM_WORLD.Allreduce(np.array([mcdc['global_tally_nuSigmaF']]),
                                 buff_nuSigmaF, MPI.SUM)
        if mcdc['technique']['IC_generator']:
            MPI.COMM_WORLD.Allreduce(
                np.array([mcdc['technique']['IC_tally_n']]),
                buff_IC_n, MPI.SUM)
            MPI.COMM_WORLD.Allreduce(
                np.array([mcdc['technique']['IC_tally_C']]),
                buff_IC_C, MPI.SUM)
            MPI.COMM_WORLD.Allreduce(
                np.array([mcdc['technique']['IC_Pmax_n']]),
                buff_Pmax_n, MPI.MAX)
            MPI.COMM_WORLD.Allreduce(
                np.array([mcdc['technique']['IC_Pmax_C']]),
                buff_Pmax_C, MPI.MAX)
...
def score_iqmc_flux(P, distance, mcdc):
    ...
    mcdc['technique']['iqmc_effective_scattering'][:, t, x, y, z] += (flux
                                                                      * SigmaS)
