...
def loop_source_async_factory():
    ...
    def loop_source(prog : numba.uintp, P: type_.particle):
        ...
        for work_idx in range(device(prog)["mpi_work_size"]):
            # Particle tracker
            #! Static Branch
            if device(prog)["setting"]["track_particle"]:
                device(prog)["particle_track_history_ID"] += 1

            # Initialize RNG wrt work index
            kernel.rng_skip_ahead_strides(work_idx, device(prog))
            ...
            if prog(mcdc)["bank_source"]["size"] == 0:
                # Sample source
                xi = kernel.rng(mcdc)
                tot = 0.0
...
def loop_particle(P, mcdc):
    ...
    while P["alive"]:
        ...
        # Mesh tally
        #! Static branch guarding body
        #! Does global accumulations
        if event & EVENT_MESH:
            kernel.mesh_crossing(P, mcdc)

        # Different Methods for shifting the particle
        # Surface crossing
        #! Seems okay. Could be broken into its own event
        ...
        # Shift particle
            ...
            #! Quite small. Probably not good as a standalone event.
        ...
        # Apply weight window
        #! Can create or destroy particles
        #! Static branch
    ...
    # Particle tracker
    if mcdc["setting"]["track_particle"]:
        #! Stores away particle into a global array
        kernel.track_particle(P, mcdc)
