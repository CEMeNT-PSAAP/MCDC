...
def make_type_source(G):
    ...
    source = np.dtype([('ID', int64),
                       ...
                       ('box_z', float64, (2,)),
                       ('ux', float64), ('uy', float64), ('uz', float64),
                       ('white_x', float64), ('white_y', float64),
                       ('white_z', float64),
...
score_x_list = ('flux_x', 'current_x', 'eddington_x', 'density_x', 'fission_x',
                'total_x')
score_y_list = ('flux_y', 'current_y', 'eddington_y', 'density_y', 'fission_y',
                'total_y')
score_z_list = ('flux_z', 'current_z', 'eddington_z', 'density_z', 'fission_z',
                'total_z')
score_t_list = ('flux_t', 'current_t', 'eddington_t', 'density_t', 'fission_t',
                'total_t')
...
def make_type_technique(card):
    ...
    # Mesh
    mesh, Nx, Ny, Nz, Nt, Nmu, N_azi =\
        make_type_mesh(card.technique['ww_mesh'])
    struct += [('ww_mesh', mesh)]
    ...
    bank_neutron_local = np.dtype([('content', neutron, (Nn_local,)),
                                   ('size', int64)])
    bank_precursor_local = np.dtype([('content', precursor, (Np_local,)),
                                     ('size', int64)])
